...
function makeTrick(cards, winnerId) {
    return {
...
describe('scoring', () => {
    test('점수 카드 A,10,K 각 1점', () => {
        ...
        expect((0, scoring_1.countTrickPoints)(trick)).toBe(3);
    });
    test('조커 포함 트릭 +1점', () => {
        const trick = makeTrick([
        ...
        expect((0, scoring_1.countTrickPoints)(trick)).toBe(2); // 조커1 + A1
    });
    test('주공팀 공약 달성 시 승리', () => {
        const bid = { playerId: 'p1', count: 13, trump: 'spade' };
        // 10트릭 중 주공팀이 13점 획득
        const tricks = Array.from({ length: 10 }, (_, i) => makeTrick([
            { playerId: 'p1', suit: 'spade', rank: 'A' },
        ...
        const result = (0, scoring_1.calculateResult)(tricks, bid, 'p1', null);
    ...
    test('주공팀 공약 달성', () => {
        ...
        const tricks = Array.from({ length: 10 }, () => makeTrick([
            { playerId: 'p1', suit: 'spade', rank: 'A' },
            { playerId: 'p2', suit: 'heart', rank: '10' },
            { playerId: 'p3', suit: 'diamond', rank: 'K' },
            { playerId: 'p4', suit: 'club', rank: '9' },
