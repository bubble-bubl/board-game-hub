...
exports.chooseCardExpert = chooseCardExpert;
exports.chooseBidExpert = chooseBidExpert;
...
function determinize(state, myId) {
    ...
    for (const tc of state.currentTrick)
        playedIds.add(tc.card.id);
    const allCards = (0, deck_1.createDeck)();
...
function randomPlayout(state) {
    let s = state;
    let iterations = 0;
    while (s.phase === 'playing' && iterations < 100) {
        const playerId = s.currentTurnId;
        const player = s.players.find((p) => p.id === playerId);
        const valid = (0, trick_1.getValidCards)(player.hand, s.currentTrick, s.trump);
        if (valid.length === 0)
            break;
        const card = valid[Math.floor(Math.random() * valid.length)];
        s = (0, game_1.applyPlayCard)(s, playerId, card);
        iterations++;
    }
    return s.winner ?? 'opposition';
}
function chooseCardExpert(validCards, state, myId) {
    if (validCards.length === 1)
        return validCards[0];
    const nodes = validCards.map((card) => ({ card, wins: 0, visits: 0 }));
    ...
    for (let d = 0; d < NUM_DETERMINIZATIONS && Date.now() - startTime < TIME_LIMIT_MS; d++) {
        ...
        for (const node of nodes) {
            ...
            for (let s = 0; s < NUM_SIMULATIONS; s++) {
                const afterPlay = (0, game_1.applyPlayCard)(detState, myId, node.card);
...
function chooseBidExpert(hand, currentBid) {
    return (0, intermediate_1.chooseBidIntermediate)(hand, currentBid);
}
